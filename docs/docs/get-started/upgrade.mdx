...
2. Fetch and apply the latest container images while preserving your OpenRAG data:
   ...
   If you run OpenRAG via the **TUI**, the `OPENRAG_VERSION` variable is automatically overwritten at runtime to match the installed Python package version.
   Therefore, the TUI always runs the container version that corresponds to the installed TUI version.
